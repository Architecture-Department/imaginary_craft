package architecture.imaginarycraft.client;

import architecture.imaginarycraft.core.ImaginaryCraft;
import net.minecraft.resources.ResourceLocation;

public final class ModGuiLayers {
	public static final ResourceLocation GUN_CHARGE_UP_HUD_LAYER_CROSSHAIR = ImaginaryCraft.modRl("gun_charge_up_hud_layer_crosshair");
	public static final ResourceLocation GUN_CHARGE_UP_HUD_LAYER_HOTBAR = ImaginaryCraft.modRl("gun_charge_up_hud_layer_hotbar");
	public static final ResourceLocation CHOP_FLAVOR = ImaginaryCraft.modRl("chop_flavor");
	public static final ResourceLocation LC_DAMAGE_SCREEN_FILTER = ImaginaryCraft.modRl("lc_damage_screen_filter");
	public static final ResourceLocation LEFT_BAR = ImaginaryCraft.modRl("left_bar");
	public static final ResourceLocation SCREEN_FILTER = ImaginaryCraft.modRl("screen_filter");
}
